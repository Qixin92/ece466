		...
		while(true)
		{
			...
			if (System.nanoTime() >= nextSendOK)
			{
				 ...
				 */
				 ...
				 //we place 2, video traffic into buffers[1], high priority traffic
				 //we place 1, poisson traffic into buffers[0], low priority
				 //
				 //so lets take high priority traffic out of the buffers[1] first
				 //and if there are none, then we take low priority traffic out of buffers[0]
				 //
				 // check priority 2 (buffer 1) first
				if ((packet = buffers[1].peek()) != null)
				{
					sendPacket(packet, startTime);
					buffers[1].removePacket();
				}
				// then check priority 1 (buffer 0) next
