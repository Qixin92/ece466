...
%token bucket output
[arrival_time, packetsize_p2, back_log, num_of_tokens] = textread('output_bucket_ex32_video.txt', '%f %f %f %f');
time_array2 = zeros(1,12000);
cumulative_arrival2 = zeros(1,12000);
...
title('Token Bucket (bucket video.txt)');
xlabel('time (in microseconds)');
ylabel('culmulative arrival (in bytes)');


%sink output
[packet_no_p3, packetsize_p3, arrival_time] = textread('output_sink_ex32_video.txt', '%f %f %f');
time_array3 = zeros(1,12000);
cumulative_arrival3 = zeros(1,12000);
...
figure(2);
subplot(1,1,1);
num_of_tokens_to_plot = zeros(1,12000);
back_log_to_plot = zeros(1,12000);
i=1;
while i<=12000
