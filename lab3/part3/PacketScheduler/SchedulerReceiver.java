	...
	public void run() {		
		...
		try {
			...
			while (true) {
				 ...
				 * elapsed time (microseconds), packet size (bytes), 
				 * backlog in buffers ordered by index in array (bytes),
				 * packet priority/class (int)
				 */
				...
				pOut.print((startTime-previousTime)/1000 + "\t" + packet.getLength() + "\t");
				...
				if (packet.getData() [0] == (byte) 1) {
					...
					if (buffers[2].addPacket(new DatagramPacket(packet.getData(), packet.getLength())) < 0) {
						System.err.println("Packet dropped (queue 3 full).");
					}
				}
				pOut.println();
				/* TODO: 
